...
`思路`：将多项式的系数视为线性表 {{< math >}}$P=((p_1,e_1),(p_2,e_2),···,(p_m,e_m))${{< /math >}} 每一项的指数 i 隐含在其系数 {{< math >}}$p_i${{< /math >}} 的序号中
...
`思路`：只存储系数不为0的项，避免浪费空间。每一项存储 `系数a[i]` 和 `指数` ，将其构成的序列记为线性表 {{< math >}}$P=((p_1,e_1),(p_2,e_2),···,(p_m,e_m))${{< /math >}}
